...
# V3: crosselling separado por tipo, lista manual, archivo o código inicial; ranking y WhatsApp por socio.
...
def mensaje_ranking_general(resumen: pd.DataFrame) -> str:
    ...
    for i, (_, r) in enumerate(resumen.iterrows(), start=1):
        ...
        lineas.append(
            ...
            f"({float(r['Cumplimiento']):.1f}%) | Cross: *{int(r['Crosselling'])}* | Faltan: *{int(r['Faltan'])}*"
        )
...
def mostrar_whatsapp(ventas: pd.DataFrame, resumen: pd.DataFrame) -> None:
    ...
    if tipo in {"Avance por socio", "Solo códigos objetivo"}:
        ...
        prefijo_key = f"ranking_{len(resumen)}_{int(resumen['Ventas objetivo'].sum())}_{int(resumen['Crosselling'].sum())}"
...
def mostrar_inicio() -> None:
    ...
    st.markdown(
        ...
        3. Revisa **Dashboard**.
        4. Entra a **WhatsApp** y selecciona el socio.
        5. Si hay códigos que deben tratarse como crosselling, agrégalos en el panel lateral.
        """
    )
