...
# =========================================================
# MÓDULO: CÓDIGOS ANULADOS
# =========================================================
def preparar_anulados(df_original: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, str | None]]:
    df = normalizar_dataframe(df_original)

    col_codigo = buscar_columna(df, ["CLIENTE_NRO", "CODIGO_CLIENTE", "COD_CLIENTE", "CODIGO", "COD", "CLIENTE"])
    col_nodo = buscar_columna(df, ["NODO_NOMBRE", "NODO", "NODO_ACTUAL", "NODO RED", "NODO_RED"])
    col_fecha = buscar_columna(df, ["MOTIVO_FECHA", "FECHA_REPORTE", "FECHA_GENERACION_OT", "FECHA", "FECHA_ESTADO"])
    col_obs = buscar_columna(df, ["CRM_OBSERVACION", "MOTIVO_OBSERVACION", "OBSERVACION", "OBS", "COMENTARIO", "COMENTARIOS"])
    col_motivo = buscar_columna(df, ["CRM_MOTIVO", "MOTIVO_ESTADO_OT", "MOTIVO_SUSPENDIDO", "MOTIVO", "MOTIVO_CUMPLIMIENTO"])
    col_estado = buscar_columna(df, ["ESTADO_OT", "ESTADO", "ESTADO_ACTUAL", "ESTADO_TRABAJO"])
    col_eh = buscar_columna(df, ["VENDEDOR_EH", "EH", "CODIGO_EH", "EHUMANO", "EH_PROMOTOR", "EJECUTIVO_EH"])
    col_socio = buscar_columna(df, ["VENDEDOR_NOMBRE", "NOMBRE_VENDEDOR", "SOCIO", "VENDEDOR", "NOMBRE_SOCIO"])
    ...
    if col_codigo is None:
        raise ValueError("No se encontró la columna de código cliente. Columnas encontradas: " + ", ".join(df.columns[:80]))

    textos_busqueda = []
    for c in [col_obs, col_motivo, col_estado]:
        if c:
            textos_busqueda.append(df[c].fillna("").astype(str))
    if textos_busqueda:
        texto_total = textos_busqueda[0]
        for serie_txt in textos_busqueda[1:]:
            texto_total = texto_total + " " + serie_txt
    else:
        texto_total = pd.Series([""] * len(df), index=df.index)

    patron_anulado = r"CODIGO\s+ANULADO|C[ÓO]DIGO\s+ANULADO|SE\s+ANULA\s+OT|ANULADO|ANULADA|ANULACION|ANULACI[ÓO]N"
    mask = texto_total.str.contains(patron_anulado, case=False, na=False, regex=True)
    anulados = df[mask].copy()

    if anulados.empty:
        detalle = pd.DataFrame(columns=["Código anulado", "Código relacionado", "Nodo", "Fecha", "EH", "Socio", "Motivo"])
        return detalle, columnas

    def extraer_codigo_anulado(row) -> str:
        obs = " ".join(str(row.get(c, "")) for c in [col_obs, col_motivo, col_estado] if c)
        m = re.search(r"C[ÓO]DIGO\s+ANULADO\s*:\s*(\d+)", obs, flags=re.I)
        if m:
            return m.group(1)
        m = re.search(r"codigo\s+(\d+).*?SE\s+ANULA\s+OT", obs, flags=re.I | re.S)
        if m:
            return m.group(1)
        return limpiar_codigo(row.get(col_codigo, ""))

    def motivo_anulacion(row) -> str:
        obs = limpiar_texto(row.get(col_obs, "")) if col_obs else ""
        motivo = limpiar_texto(row.get(col_motivo, "")) if col_motivo else ""
        estado = limpiar_texto(row.get(col_estado, "")) if col_estado else ""
        texto = obs or motivo or estado or "Sin motivo registrado"
        texto = re.sub(r"\s+", " ", texto).strip()
        return texto

    detalle = pd.DataFrame({
        "Código anulado": anulados.apply(extraer_codigo_anulado, axis=1),
        "Código relacionado": anulados[col_codigo].apply(limpiar_codigo) if col_codigo else "",
        "Nodo": anulados[col_nodo].apply(limpiar_texto).str.upper() if col_nodo else "",
        "Fecha": parsear_fecha(anulados[col_fecha]) if col_fecha else "",
        "EH": anulados[col_eh].apply(limpiar_eh) if col_eh else "",
        "Socio": anulados[col_socio].apply(limpiar_texto) if col_socio else "",
        "Motivo": anulados.apply(motivo_anulacion, axis=1),
    })
    detalle = detalle.drop_duplicates(subset=["Código anulado", "Fecha", "Motivo"]).reset_index(drop=True)
    detalle = detalle.sort_values(["Fecha", "Código anulado"], ascending=[True, True]).reset_index(drop=True)
    return detalle, columnas


def mensaje_anulados(detalle: pd.DataFrame) -> str:
    lineas = [
        "🚫 *CÓDIGOS ANULADOS*",
        f"🔢 Total: *{len(detalle)}*",
        "",
    ...
    if detalle.empty:
        lineas.append("✅ No se encontraron códigos anulados en el archivo.")
        return "\n".join(lineas)

    lineas.append("📋 *Código | Fecha | Motivo*")
    for i, (_, r) in enumerate(detalle.iterrows(), start=1):
        codigo = r.get("Código anulado", "") or "S/D"
        fecha = r.get("Fecha", "")
        fecha_txt = pd.to_datetime(fecha).strftime("%d/%m/%Y") if pd.notna(fecha) and fecha != "" else "S/F"
        motivo = str(r.get("Motivo", "Sin motivo registrado")).strip()
        lineas.append(f"{i}. {codigo} | {fecha_txt} | {motivo}")
    return "\n".join(lineas)


def mostrar_anulados() -> None:
    st.title("🚫 Códigos anulados")
    st.caption("Carga el archivo GENERADAS o una base similar. La consulta detecta códigos anulados, fecha y motivo.")

    archivo = st.file_uploader("📤 Subir archivo", type=["csv", "xlsx", "xls"], key="anulados_uploader")
    if archivo is None:
        st.info("Sube el archivo para consultar códigos anulados.")
        return

    try:
        df_original = leer_archivo(archivo)
        detalle, columnas = preparar_anulados(df_original)
    except Exception as exc:
        st.error(f"No se pudo procesar el archivo: {exc}")
        return

    st.metric("Códigos anulados encontrados", len(detalle))

    with st.expander("Diagnóstico de columnas detectadas"):
        st.json({k: str(v) for k, v in columnas.items()})

    st.subheader("📲 Mensaje WhatsApp")
    texto = mensaje_anulados(detalle)
    st.text_area("Copiar mensaje", texto, height=320, key="anulados_msg")
    st.markdown(f"[📲 Enviar por WhatsApp]({whatsapp_link(texto)})")

    st.subheader("📋 Detalle")
    if detalle.empty:
        st.warning("No se encontraron anulados en el archivo cargado.")
    else:
        st.dataframe(detalle[["Código anulado", "Fecha", "Motivo", "Código relacionado", "Nodo", "EH", "Socio"]], use_container_width=True, hide_index=True)
        excel = excel_bytes({"Anulados": detalle})
        st.download_button(
            "⬇️ Descargar Excel",
            data=excel,
            file_name=f"codigos_anulados_{hoy_bolivia().strftime('%Y%m%d')}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            key="anulados_excel",
        )
